...
int PID::calcPID(int pwm[], int throttle, float desired_pitch_angle, float desired_roll_angle, float desired_yaw, long dlay)
{
    ...
    pitch_d = kd * (pitch_error - prev_pitch_err) / sampleRate;
    ...
    roll_d = kd * (roll_error - prev_roll_err) / sampleRate;
    ...
    prev_pitch_err = pitch_error;
    prev_roll_err = roll_error;
    ...
    pwm[3] = constrain(throttle + pitch_pid + roll_pid - yaw_pid, 1000, 2000);
    pwm[2] = constrain(throttle + pitch_pid - roll_pid + yaw_pid, 1000, 2000);
    pwm[1] = constrain(throttle - pitch_pid - roll_pid - yaw_pid, 1000, 2000);
    pwm[0] = constrain(throttle - pitch_pid + roll_pid + yaw_pid, 1000, 2000);
