...
int GUI::highlightItem(String items[], int item, int prev_item, int item_count, bool vertical)
{
  ...
  else // scroll horizontally
  {
    ...
    if (prev_item != -1)
      GUI::displayInfo("-> ", 1, w_offset + (w_space + 10) * prev_item, init_y + incr_y * (sub_list_count + 1), ST77XX_CYAN, ST77XX_BLACK, true, false);
    GUI::displayInfo("-> ", 1, w_offset + (w_space + 10) * item, init_y + incr_y * (sub_list_count + 1), ST77XX_BLACK, ST77XX_WHITE, true, false);
  }
  return 1;
}
